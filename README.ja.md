...
アルゴリズムは `ReadOnlySpan<char>` 上で動作し、マネージド割り当てを一切行いません。ガベージコレクターは走査を観測せず、ライブラリ全体が Native AOT に対応します。
...
公開面はモダンな C# です。テキストは `ReadOnlySpan<char>` として渡します。改行機会は `ref struct` の列挙子が生成するため、文書全体をヒープ負荷なしで走査できます。各改行機会は UTF-16 オフセットと種別を保持する `readonly struct` です。任意のコードポイントの改行クラスは静的メソッド一つで取得できます。状態を持つ走査器は公開しません。
...
複合文脈クラスの既定解決は独立した段階です。将来、言語固有の解析器が非tailorable規則へ触れずにこれを置換できます。初版は既定解決のみを公開します。
...
走査器は `ref struct` です。状態はすべてスタック上にあり、分類データは静的な読み取り専用メモリで、テキストの走査中に配列やオブジェクトを割り当てません。分類はブロックを重複排除した二段トライで、`ReadOnlySpan<byte>` として出力します。実行時はこれをアセンブリからヒープコピーなしで直接参照します。

マネージド割り当てが無いことは主張ではなく計測です。`GC.GetAllocatedBytesForCurrentThread` は、多言語サンプルの列挙と Unicode 範囲の全コードポイントの分類にわたり、増分ゼロバイトを報告します。
...
テストプロジェクトは 43 個のテストを含み、すべて合格します。適合テストだけで公式ファイルの全 19338 ケースを検査します。残るテストは、空文字列・単一文字・強制改行と必須改行・復帰改行の対・追加面のオフセット・Emoji の Zero Width Joiner 列・Regional Indicator の対・孤立サロゲート・全コード空間・マネージド割り当ての不在を網羅します。
...
| メンバー | 説明 |
|---|---|
| `LineBreaker.Enumerate(ReadOnlySpan<char>)` | テキストの改行機会を走査する `ref struct` 列挙子を返します。 |
| `LineBreakEnumerator.GetEnumerator()` | 列挙子自身を返し、`foreach` で直接使えます。 |
| `LineBreakEnumerator.MoveNext()` | 次の改行機会へ進み、機会があるかを報告します。 |
| `LineBreakEnumerator.Current` | 現在位置の改行機会です。 |

列挙子は改行を許すか要求する位置ごとに機会を一つ返します。改行が禁止される位置は飛ばします。最後の機会はテキスト長に等しいオフセットにあり、常に必須です。
...
- 複合文脈クラスは規則 LB1 の既定規則で解決します。既定規則は一般カテゴリにより結合印か英字を割り当て、そうした文字の連なりの内部には空白以外で改行を挿入しません。タイ・ラオ・クメール・ビルマなどの辞書分割は行いません。後から解析器を追加できるよう、解決は独立した内部段階にしています。
...
- 入力エンコーディング: 走査器は UTF-16 上で動作し、オフセットを UTF-16 コード単位の添字として報告します。サロゲート対は高位サロゲートのオフセットにある単一のスカラー値で、孤立サロゲートはサロゲートクラスの単一の単位として扱います。
- 決定性: 走査は繰り返し実行しても同一の出力を生成し、呼び出し間で状態を保持しません。列挙子は呼び出し側が所有するスパン上の `ref struct` のため、別々の列挙は独立し、インスタンスを共有しません。
- 解決の接合点: 規則 LB1 の解決は規則エンジンから分離した単一のメソッドです。非tailorable規則を保ったまま、将来の版で複合文脈の解決を置換できます。
